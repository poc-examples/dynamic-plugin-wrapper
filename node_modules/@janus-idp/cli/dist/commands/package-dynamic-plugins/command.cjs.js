...
async function command(opts) {
  ...
  if (!exportTo) {
    try {
      await tasks.Task.forCommand(`${containerTool} --version`);
    } catch (e) {
      tasks.Task.error(
...
Make sure that ${containerTool} is installed and available in your PATH.`
  ...
  const workspacePackageRole = cliNode.PackageRoles.detectRoleFromPackage(workspacePackage);
  const workspacePackageRoleInfo = workspacePackageRole !== void 0 ? cliNode.PackageRoles.getRoleInfo(workspacePackageRole) : void 0;
  const isMonoRepo = typeof workspacePackage.workspaces.packages !== "undefined";
  ...
  try {
    for (const pluginPkg of packages) {
      ...
      const packageName = packageJson.name.replace(/^@/, "").replace(/\//, "-") + ((packageRoleInfo && packageRoleInfo.platform) === "node" ? "-dynamic" : "");
      const distDirectory = path__default.default.join(packageDirectory, "dist-dynamic");
      const targetDirectory = path__default.default.join(tmpDir, packageName);
      tasks.Task.log(`Copying '${distDirectory}' to '${targetDirectory}`);
      try {
        fs__default.default.cpSync(distDirectory, targetDirectory, {
          recursive: true,
          dereference: true
        });
        const {
    ...
    if (exportTo) {
      ...
      tasks.Task.log(`Creating image using ${containerTool}`);
      await tasks.Task.forCommand(
        `echo "from scratch
COPY . .
